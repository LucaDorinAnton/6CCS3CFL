...
def PLUS(r: Rexp) = r ~ r.%
def OPT(r: Rexp) = ONE | r
def RANGE(s: String) : Rexp = s.toList match {
  case Nil => ZERO
  case head::rest =>  CHAR(head) | RANGE(rest.mkString)
}  

val LOWER = RANGE(('a' to 'z').toList.mkString)
val UPPER = RANGE(('A' to 'Z').toList.mkString)
val LETTERS = LOWER | UPPER
val SYM = LETTERS | "_"
val DIGIT = RANGE(('0' to '9').toList.mkString)
val ID = LETTERS ~ (SYM | DIGIT).% 
val INTEGER = OPT("-") ~ PLUS(DIGIT)
val FLOATING = OPT("-") ~ PLUS(DIGIT) ~ "." ~ PLUS(DIGIT) 
val KEYWORD : Rexp = "if" | "then" | "else" | "def" | "val"
val TYPE_QUAL : Rexp = "Int" | "Double" | "Void"
val SEMI: Rexp = ";"
val OP: Rexp = "=" | "==" | "-" | "+" | "*" | "/" | "!=" | "<" | ">" | "<=" | ">=" | ":"
val WHITESPACE = PLUS(" " | "\n" | "\t")
val RPAREN: Rexp = ")"
...
val token : PartialFunction[(String, String), Token] = {
  ...
  case ("pl", _) => T_LPAREN
  case ("pr", _) => T_RPAREN
  case ("bl", _) => T_LBRACKET
  case ("br", _) => T_RBRACKET
}
